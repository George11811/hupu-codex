...
针对 `../hupu-codex.user.js` 的七层验证（共 318 条断言）。全部**离线**跑：
...
依赖项禁用与压暗）、页面透明度滑杆（`--hpcx-chrome-alpha` 换算、即时预览、落盘、
CSS 里只对 `.hpcx-rail, .hpcx-main` 生效）、侧边栏模式（离开页面自动伪装、回来恢复、
鼠标没动时补发的 enter 不恢复、后代元素上的 leave 不算离开、手动按出来的伪装不被撤掉、
面板开着时不触发、关掉后不再触发、「回到页面自动恢复」关掉后不恢复但能再藏）、
草稿板（输入落盘、点「回复」写入引用、markdown 工具栏与预览、清空）、
引用卡片与思考块折叠、灯箱开关、rail 分类折叠、`Ctrl+K` 搜索。

指针进出的辅助函数照真实浏览器的三件事来派发：

1. **带非零坐标**：脚本忽略「合成事件 + 0,0 坐标」（那种事件会把刚藏好的伪装立刻放回来）；
2. **`pointerEnter()` 先发一个 `pointermove`**：脚本用「指针动过」判断人是否回来了；
3. **只从 `<html>` 自己派发**：真实浏览器里 pointerleave / pointerenter 会在每个后代元素上
   各来一次，脚本按 `e.target` 过滤 —— `childPointerLeave()` 就是专门从后代节点派发的那种，
   用来断言它**不被**当成「离开页面」。

事件类型也按 jsdom 实际情况选（它没有 `PointerEvent`，脚本自己会退化成
...
### `browser.js` —— 真实浏览器（88 条，本机 Chrome/Edge）
...
伪装视图铺满视口且正文不可见、**页面透明度只作用在 rail / 主区**（`html`/`body`、
伪装视图与设置面板必须还是 `opacity: 1`）、**侧边栏模式的真实鼠标离开 / 返回**，
以及关掉「回到页面自动恢复」之后回来仍停在伪装视图、但应急键仍能退出。

侧边栏模式那组必须用真实鼠标移动（`page.mouse.move` 到视口外），不能
`dispatchEvent(new PointerEvent("pointerleave"))`：合成事件的坐标是 `0,0`，
脚本会主动忽略，而且盖不到真实浏览器的三个坑（进入伪装时的 `blur()` 会引发补发的
`pointerenter`、`pointerenter` 早于第一个 `pointermove`、`pointerleave` 在每个后代元素上
各来一次）—— 用合成事件测会得到「藏了又被放回来」或「回来了却还藏着」的假象，
让人以为是实现坏了。三个坑的完整说明在主 README 第 21 条。
